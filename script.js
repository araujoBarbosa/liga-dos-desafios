// ===============================
//  LIGA DOS DESAFIOS – script.js
//  (substitua TODO o conteúdo atual por este)
// ===============================

// ====== IMPORTAÇÕES DO FIREBASE (SDK MODERNO) ======
...
const desafios = [
  ...
  { id: 2, titulo: "Dancinha em família", nivel: "💚 Fácil", pontos: 10, tipo: "Vídeo", emoji: "💃", descricao: "Gravem uma dança simples juntos." },
  ...
  { id: 6, titulo: "Bandeira do time", nivel: "🟡 Médio", pontos: 30, tipo: "Foto", emoji: "🏳️", descricao: "Criem uma bandeira que representa o time." },
...
// ====== ELEMENTOS IMPORTANTES DE UI (se existirem) ======
...
const statusMembrosEl = document.getElementById("statusMembros");
const tituloTimePainelEl = document.getElementById("tituloTimePainel");
...
function carregarDesafioAtual() {
  ...
  const emojiAtual = document.getElementById("emojiAtual");
  const tituloDesafioAtual = document.getElementById("tituloDesafioAtual");
  const nivelDesafioAtual = document.getElementById("nivelDesafioAtual");
  const pontosDesafioAtual = document.getElementById("pontosDesafioAtual");
  const descricaoDesafioAtual = document.getElementById("descricaoDesafioAtual");
  const contadorDesafios = document.getElementById("contadorDesafios");

  if (emojiAtual) emojiAtual.textContent = d.emoji || "🎯";
  if (tituloDesafioAtual) tituloDesafioAtual.textContent = d.titulo;
  if (nivelDesafioAtual) nivelDesafioAtual.textContent = `${d.nivel} • ${d.pontos} pts`;
  if (pontosDesafioAtual) pontosDesafioAtual.textContent = `${d.pontos} pts`;
  if (descricaoDesafioAtual) descricaoDesafioAtual.textContent = d.descricao;
  if (contadorDesafios) contadorDesafios.textContent = `${desafioAtual} / ${totalDesafios}`;
...
// ------ RENDERIZA STATUS DOS MEMBROS ------
function renderizarStatusMembros() {
  if (!statusMembrosEl) return;
  statusMembrosEl.innerHTML = "";
  membros.forEach((m) => {
    const fez = (m.concluidos || 0) >= desafioAtual;
    const eliminado = (m.faltas || 0) >= 3;
    const li = document.createElement("li");
    li.textContent = `${m.nome}: ${
      fez ? "✔️ Fez" : eliminado ? "❌ Eliminado" : "⏳ Aguardando"
    }`;
    statusMembrosEl.appendChild(li);
  });
}

// ====== MARCAR CONCLUSÃO DE DESAFIO ======
async function marcarConclusao(membroNome, registro = {}) {
  if (!timeCodigo) {
    alert("Entre ou crie um time para enviar.");
    return;
  }
  const id = keyFromNome(membroNome);
  const membroRef = ref(db, `times/${timeCodigo}/membros/${id}`);
  const atual =
    membros.find((m) => keyFromNome(m.nome) === id) || {
      concluuidos: 0,
      faltas: 0,
      nome: membroNome,
    };

  const novosConcluidos = (atual.concluidos || 0) + 1;

  await set(membroRef, {
    ...atual,
    nome: membroNome,
    concluuidos: novosConcluidos,
  ...
  const todosConcluidos = membros.every(
    (m) =>
      (m.concluidos || 0) >= desafioAtual ||
      (m.faltas || 0) >= 3 ||
      keyFromNome(m.nome) === id
  );
    ...
    await Promise.all(
      membros.map((m) => {
        if ((m.concluidos || 0) < desafioAtual) {
          ...
          return set(ref(db, `times/${timeCodigo}/membros/${mid}`), {
            ...m,
            faltas: (m.faltas || 0) + 1,
          });
        }
...
async function uploadMidia(tipo) {
  ...
  const caminho = `desafios/${timeCodigo}/${keyFromNome(
    apelido
  )}/${Date.now()}-${file.name}`;
...
// ====== MONITORAMENTO DO TIME NO FIREBASE ======
function conectarAoTime(codigo) {
  timeCodigo = codigo;
  ...
  onValue(ref(db, `times/${codigo}/nome`), (snap) => {
    const nome = snap.val();
    if (tituloTimePainelEl && nome) {
      tituloTimePainelEl.textContent = `Time: ${nome}`;
    }
  });
  ...
  onValue(ref(db, `times/${codigo}/historico`), (snap) => {
    ...
    const pontos = desafios
      .slice(0, concluidos)
      .reduce((s, d) => s + d.pontos, 0);
    const pct = Math.round((concluidos / desafios.length) * 100);

    if (barraProgressoEl) barraProgressoEl.style.width = pct + "%";
    if (textoDesafiosConcluidosEl)
      textoDesafiosConcluidosEl.textContent = `${concluidos} / ${desafios.length} desafios`;
    if (textoPercentualProgressoEl)
      textoPercentualProgressoEl.textContent = `${pct}%`;
    if (headerProgressoEl) headerProgressoEl.textContent = `${pct}%`;
    if (pillPontosTotaisEl) pillPontosTotaisEl.textContent = `${pontos} pts`;
    if (miniDesafioAtualEl)
      miniDesafioAtualEl.textContent = `#${Math.min(
        concluidos + 1,
        desafios.length
      )} ${desafios[Math.min(concluidos, desafios.length - 1)].titulo}`;
  });
}
...
async function criarTimeFirebase(nomeTime) {
  ...
  await set(
    ref(db, `times/${codigo}/membros/${keyFromNome(apelido)}`),
    {
      nome: apelido,
      concluuidos: 0,
      faltas: 0,
...
function configurarChatFirebase() {
  ...
  onChildAdded(chatRef, (snap) => {
    ...
    const isTexto =
      (dados.tipo === "texto" && dados.texto) ||
      (!!dados.texto && !dados.tipo);
    ...
    const msg = {
      ...
      corAvatar:
        dados.corAvatar ||
        gerarCorAPartirDoNome(dados.autorNome || "Jogador"),
...
// ===== ÁUDIO (MANDA PRO CHAT COMO MENSAGEM DE ÁUDIO) =====
let mediaRecorder;
let chunks = [];
  ...
  try {
    ...
    mediaRecorder.onstop = async () => {
      try {
        ...
        const arquivoRef = storageRef(
          storage,
          "audios/" + Date.now() + ".webm"
        );
...
btnAudio?.addEventListener("touchend", () => {
  if (mediaRecorder && mediaRecorder.state === "recording") mediaRecorder.stop();
});

// ====== BIND BOTÕES (TIME / UPLOAD) ======
document
  .getElementById("btnContinuarDashboard")
  ?.addEventListener("click", () => {
    const nomeTime = document
...
document
  .querySelector('#screen-time .btn-entrar-time')
  ?.addEventListener("click", () => {
    const cod = document
      .getElementById("codigoTime")
      ?.value?.trim()
      ?.toUpperCase();
    if (!cod) {
      alert("Informe o código do time");
      return;
    }
...
document
  .getElementById("btnEnviarFoto")
  ?.addEventListener("click", () => uploadMidia("foto"));
document
  .getElementById("btnEnviarVideo")
  ?.addEventListener("click", () => uploadMidia("video"));
